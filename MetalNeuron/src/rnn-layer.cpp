...
// Using kernel names "forward_rnn" and "learn_rnn".
...
RNNLayer::RNNLayer(int inputDim, int hiddenDim)
    : inputDim_(inputDim), hiddenDim_(hiddenDim),
      ...
      bufferW_xh_(nullptr), bufferW_hh_(nullptr), bufferBias_(nullptr), bufferError_(nullptr),
...
void RNNLayer::buildPipeline(MTL::Device* device, MTL::Library* library) {
    ...
    if (!forwardPipelineState_) {
        std::cerr << "Error building forward pipeline state for RNNLayer: "
                  << error->localizedDescription()->utf8String() << std::endl;
        assert(false);
    }
    backwardPipelineState_ = device->newComputePipelineState(backwardFunc, &error);
    if (!backwardPipelineState_) {
        std::cerr << "Error building backward pipeline state for RNNLayer: "
                  << error->localizedDescription()->utf8String() << std::endl;
        assert(false);
    }
    forwardFunc->release();
...
void RNNLayer::buildBuffers(MTL::Device* device) {
    ...
    float* bias = static_cast<float*>(bufferBias_->contents());
    memset(bias, 0, hiddenDim_ * sizeof(float));
    bufferBias_->didModifyRange(NS::Range::Make(0, bufferBias_->length()));
    
    // Allocate error buffer for learning.
...
void RNNLayer::forward(MTL::CommandBuffer* cmdBuf) {
    ...
    // Bind buffers:
    // 0: input, 1: previous hidden, 2: current hidden,
    // 3: W_xh, 4: W_hh, 5: bias, 6: inputDim, 7: hiddenDim.
...
void RNNLayer::backward(MTL::CommandBuffer* cmdBuf) {
    ...
    // Bind buffers for learn_rnn:
    // 0: input, 1: previous hidden, 2: W_xh, 3: W_hh,
    // 4: bias, 5: current hidden, 6: error,
    // 7: inputDim, 8: hiddenDim.
    encoder->setBuffer(bufferInput_, 0, 0);
    encoder->setBuffer(bufferHiddenPrev_, 0, 1);
    ...
    encoder->setBuffer(bufferError_, 0, 6);
    encoder->setBytes(&inputDim_, sizeof(int), 7);
    encoder->setBytes(&hiddenDim_, sizeof(int), 8);
    uint32_t threads = hiddenDim_;
    MTL::Size threadsPerThreadgroup = MTL::Size{ std::min(threads, 1024u), 1, 1 };
    MTL::Size threadgroups = MTL::Size{ (threads + 1023) / 1024, 1, 1 };
    encoder->dispatchThreadgroups(threadgroups, threadsPerThreadgroup);
