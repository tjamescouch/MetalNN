...
#include "sequence-to-vector-layer.h"
#include "training-manager.h"


SequenceToVectorLayer::SequenceToVectorLayer(int inputDim, int outputDim, int batchSize, int sequenceLength) :
...
SequenceToVectorLayer::~SequenceToVectorLayer() {
...
void SequenceToVectorLayer::buildPipeline(MTL::Device* device, MTL::Library* library) {
...
void SequenceToVectorLayer::buildBuffers(MTL::Device* device) {
...
void SequenceToVectorLayer::forward(MTL::CommandBuffer* cmdBuf, int batchSize) {
    for(int t = 0; t < sequenceLength_; ++t) {
...
void SequenceToVectorLayer::backward(MTL::CommandBuffer* cmdBuf, int batchSize) {
    for(int t = 0; t < sequenceLength_; ++t) {
        auto encoder = cmdBuf->computeCommandEncoder();
...
void SequenceToVectorLayer::setInputBufferAt(BufferType type, int timestep, MTL::Buffer* buffer) {
    assert(buffer && "Setting input buffer to NULL");
    inputBuffers_[type][timestep] = buffer;
}

MTL::Buffer* SequenceToVectorLayer::getOutputBufferAt(BufferType type, int timestep) {
    return outputBuffers_[type][timestep];
}

void SequenceToVectorLayer::setOutputBufferAt(BufferType type, int timestep, MTL::Buffer* buffer) {
    outputBuffers_[type][timestep] = buffer;
}

MTL::Buffer* SequenceToVectorLayer::getInputBufferAt(BufferType type, int timestep) {
    return inputBuffers_[type][timestep];
}

void SequenceToVectorLayer::connectForwardConnections(Layer* previousLayer, Layer* inputLayer,
...
void SequenceToVectorLayer::connectBackwardConnections(Layer* prevLayer,
                                   Layer* inputLayer,
...
void SequenceToVectorLayer::saveParameters(std::ostream& os) const {
    // No parameters to save
}

void SequenceToVectorLayer::loadParameters(std::istream& is) {
    // No parameters to load
}


void SequenceToVectorLayer::onForwardComplete(MTL::CommandQueue* _pCommandQueue, int batchSize) {
}

void SequenceToVectorLayer::onBackwardComplete(MTL::CommandQueue* _pCommandQueue, int batchSize) {
}
